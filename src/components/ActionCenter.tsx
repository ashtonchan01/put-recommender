    ...
    <div className="flex-1 overflow-y-auto px-4 lg:px-6 py-4 flex flex-col gap-4 max-w-4xl w-full">
      ...
      {syncData && (
        <div className="glass-card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {lastSyncLabel && <span className="text-xs text-neutral-500">Synced {lastSyncLabel}</span>}
          </div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <p className="text-neutral-500 text-xs">YTD</p>
              <p className="text-sky-400 font-bold text-lg">{fmtK(ytdPremium)}</p>
            </div>
            <div>
              <p className="text-neutral-500 text-xs">{thisMonth ? new Date().toLocaleString("default", { month: "short" }) : "This Month"}</p>
      ...
      {openCycles.length > 0 && (
        <div className="flex flex-col gap-2">
          <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">Open Wheels ({openCycles.length})</h3>
            ...
            return (
              <div key={c.id} className="glass-card p-3.5">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs text-neutral-500">${c.totalPremiumCollected.toFixed(0)} collected</span>
                </div>
                {/* Stage progression */}
                <div className="flex items-center gap-0.5">
                  {STAGES.map((stage, i) => (
                    <div key={stage} className="flex items-center gap-0.5">
                      ...
                      {i < STAGES.length - 1 && (
                        <div className={`w-1.5 h-px ${i < activeStage ? "bg-emerald-500/40" : "bg-neutral-700"}`} />
                      )}
                    </div>
                  ))}
                </div>
          ...
          {openCycles.length > 5 && (
            <p className="text-xs text-neutral-500 text-center">+{openCycles.length - 5} more</p>
          )}
        </div>
      )}
      ...
      {!syncData && !syncing && (
        <div className="glass-card p-6 text-center flex flex-col gap-3">
          ...
          <p className="text-neutral-400 text-sm">Go to Settings → IBKR to add your Flex token and Query ID.</p>
        </div>
      )}
      ...
      {syncData && actions.length === 0 && !syncing && (
        <div className="glass-card p-6 text-center flex flex-col gap-3">
          ...
          {!hasScanned && <p className="text-neutral-400 text-sm">Run a scan to find CSP opportunities.</p>}
        </div>
      )}
      ...
      {(syncData || hasScanned) && (
        <div className="glass-card p-4 flex flex-col gap-2">
          <p className="text-neutral-500 text-xs font-semibold uppercase tracking-wider mb-1">Active Rules</p>
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-neutral-600">({actions.length})</span>
      </div>
...
function ActionCard({ action: a }: { action: Action }) {
  ...
  const typeColor = actionTypeColor[a.type] ?? "text-neutral-300";
    ...
    <div className={`rounded-xl border bg-gradient-to-br p-4 flex flex-col gap-2 ${cfg.border} ${cfg.bg}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-sm text-neutral-200">{a.headline}</span>
        </div>
        {a.pnlDollars !== undefined && (
          <div className="shrink-0 text-right">
            ...
            {a.pnlPct !== undefined && <div className="text-xs text-neutral-500">{a.pnlPct.toFixed(0)}% of max</div>}
          </div>
        )}
      </div>
      <div className="text-xs text-neutral-400 border-t border-white/5 pt-2">{a.detail}</div>
      {a.scanPick && (
        <div className="grid grid-cols-3 gap-1 text-center bg-neutral-900/40 rounded-lg p-2">
...
function StatCard({ label, value, sub, color = "slate" }: { label: string; value: string; sub?: string; color?: string }) {
  ...
  const textColors: Record<string, string> = {
    emerald: "text-emerald-400", rose: "text-rose-400", sky: "text-sky-400", violet: "text-violet-400", slate: "text-neutral-300",
  };
  return (
